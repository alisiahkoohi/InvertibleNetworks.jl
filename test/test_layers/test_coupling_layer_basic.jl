...
# Input images
Xa = randn(Float32, nx, ny, nz, Int(k/2), batchsize)
Xb = randn(Float32, nx, ny, nz, Int(k/2), batchsize)
Xa0 = randn(Float32, nx, ny, nz, Int(k/2), batchsize)
Xb0 = randn(Float32, nx, ny, nz, Int(k/2), batchsize)
dXa = Xa - Xa0
dXb = Xb - Xb0

# 1x1 convolution and residual blocks
RB = ResidualBlock(nx, ny, nz, n_in, n_hidden, batchsize; fan=true)
L = CouplingLayerBasic(RB; logdet=true)
...
# Invertible layers
RB0 = ResidualBlock(nx, ny, nz, n_in, n_hidden, batchsize; fan=true)
L = CouplingLayerBasic(RB; logdet=true)
L01 = CouplingLayerBasic(RB; logdet=true)
L02 = CouplingLayerBasic(RB0; logdet=true)
